...
- [ ] **Step 1.5: End-to-end flow verification (user-mandated).** One script, `scripts/check-graph-e2e.ts`, that proves the whole circle with a served app: (1) serve the export (`npx serve out` equivalent via node http static, or `npx next start` on the non-export build) on a local port; (2) HTTP assertions — `GET /graph` → 200 and the HTML contains the drop-zone entry markup; `GET /graph/graph.json` → 200, `nodes.length === 312`, `meta.stages` + `meta.guestIds` present and internally consistent (`stages.unique === nodes.length`); `GET /graph/people/<tj-personId>.json` → 200; (3) **the closing loop**: from TJ's person file take his top seek edge's receipt, verify `receipt.theirs.quote`/`receipt.yours.quote` appear VERBATIM in the cited guests' fields in the raw CSV (re-loaded via `loadGuests`) — a rendered receipt traced back to the source sheet in one assertion chain. Exit non-zero on any failure, named errors throughout.
