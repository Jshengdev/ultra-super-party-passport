...
| **v2 pipeline** | `lib/guests.ts` → `scripts/ingest-guests.ts` → `lib/conviction.ts`/`scripts/enrich-convictions.ts` → `lib/matches.ts`/`scripts/enrich-matches.ts` → `lib/layout.ts`/`scripts/emit-graph.ts` → `public/graph/*`; emit-side derivations: `lib/places.ts` (hometown→metro + vendored `city-table` coords → graph.json `places[]`), `lib/taste.ts` (exact-match twins on favorite/inspiration → the record's `taste` block) | real guest list (312) → baked graph artifacts |
